...
<form>
  ...
  class Edge {
      constructor(target, index, source, type) {
          ...
          this.visible = false;
          this.curvedLineGen = line().curve(basis$2);
          this.rectangularLineGen = line().curve(curveLinear);
      }
      stringID() {
      ...
      generatePath(graph, showTypes) {
          ...
          const result = [
              [outputX, outputY],
              [outputX, outputApproach],
              [horizontalPos, outputApproach]
          ];
          if (horizontalPos != inputX) {
              result.push([horizontalPos, inputApproach]);
          }
          else {
              if (inputApproach < outputApproach) {
